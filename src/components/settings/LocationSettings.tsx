...
export const LocationSettings = () => {
    ...
    useEffect(() => {
        const qItems = query(collection(db, 'locations'));
    ...
    const handleSave = async () => {
        ...
        try {
            if (editingItem) {
                await updateDoc(doc(db, 'locations', editingItem.id), {
                    name: itemName.trim()
                });
            } else {
                    ...
                    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200">
                        <div className="p-6">
                            ...
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Nome do Banco/Prateleira</label>
                                    <input
                                        type="text"
                                        value={itemName}
