...
import { Package, Plus, Edit2, Trash2, X, Check, Loader2 } from 'lucide-react';
...
export const ConservationSettings = () => {
    const [conservations, setConservations] = useState<Conservation[]>([]);
    const [showModal, setShowModal] = useState(false);
    const [editingItem, setEditingItem] = useState<Conservation | null>(null);
    const [itemName, setItemName] = useState('');
    const [isSaving, setIsSaving] = useState(false);

    useEffect(() => {
        const qItems = query(collection(db, 'conservations'));
    ...
    const handleSave = async () => {
        ...
        try {
            if (editingItem) {
                await updateDoc(doc(db, 'conservations', editingItem.id), {
                    name: itemName.trim()
                });
            } else {
                await setDoc(doc(collection(db, 'conservations')), {
                    name: itemName.trim(),
                    createdAt: new Date()
                });
            }
            ...
            console.error('Erro ao salvar conservação:', error);
            alert('Erro ao salvar conservação. Tente novamente.');
        } finally {
            setIsSaving(false);
        }
    };
    ...
    const handleDelete = async (id: string) => {
        if (window.confirm('Tem certeza que deseja excluir esta conservação?')) {
            try {
                await deleteDoc(doc(db, 'conservations', id));
            } catch (error) {
                console.error('Erro ao deletar conservação:', error);
            }
        }
    };
                ...
                <div className="divide-y divide-slate-100 dark:divide-slate-800/60 p-2">
                    {conservations.length === 0 ? (
                        <div className="p-12 text-center">
                            ...
                            <p className="text-slate-500 font-medium">Nenhuma conservação cadastrada.</p>
                        </div>
                    ) : (
                        ...
                        <div className="p-6">
                            <div className="flex justify-between items-center mb-6">
                                <h2 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                                    ...
                                    {editingItem ? 'Editar Conservação' : 'Nova Conservação'}
                                </h2>
                            ...
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Nome da Conservação</label>
                                    <input
                                        type="text"
                                        value={itemName}
                            ...
                            <div className="mt-8">
                                ...
                                >
                                    ...
                                    {isSaving ? 'Salvando...' : 'Salvar Conservação'}
                                </button>
                            </div>
                        </div>
                    </div>
